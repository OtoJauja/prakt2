...
# Putting in the NASA API key 
nasa_api_key = "ZIYOMSRLZ5B2lkb8Z3qWZrQHMgXTBx9LftVMYKar"
...
# Printing out the data requested from NASA
print("Response status code: " + str(r.status_code))
print("Response headers: " + str(r.headers))
print("Response content: " + str(r.text))
# If r.status_code = 200, then they get parsed
if r.status_code == 200:
	# Parsing the response as a json structure
	json_data = json.loads(r.text)
	# Initialising safe and dangerous asteroid arrays
	ast_safe = []
	ast_hazardous = []
	...
	if 'element_count' in json_data:
		...
		# If it does exist then the data gets processed
		if ast_count > 0:
			for val in json_data['near_earth_objects'][request_date]:
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
					tmp_ast_name = val['name'] # Getting asteroid name
					tmp_ast_nasa_jpl_url = val['nasa_jpl_url'] # Getting url of asteroid desription
					if 'kilometers' in val['estimated_diameter']: # Getting diameter values
					...
					if len(val['close_approach_data']) > 0: # loopS through the array rather than read the first element
						...
						tmp_ast_speed = -1
						tmp_ast_miss_dist = -1
					# Prints out the info about the asteroids
					print("------------------------------------------------------- >>")
					...
					# Adding asteroid data to the array depending on the asteroid type
		...
		# If no asteroids were found this gets printed out
		else:
			print("No asteroids are going to hit earth today")
	# Prints out the number of hazardous asteroids and safe asteroids
	print("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe)))
	
	if len(ast_hazardous) > 0:
		# Sorts array by time theasteroid will hit
		...
		# Sorts array by the distance of the asteroid from the earth
...
# Prints out this if not able to get a response from API
else:
